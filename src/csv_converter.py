...
def determine_grid_size(
    df: pd.DataFrame, grid_size: Optional[Tuple[int, int]]
) -> Tuple[int, int]:
    """
...
def create_grid_for_step(
    step_data: pd.DataFrame, grid_shape: Tuple[int, int], value_column: str
) -> np.ndarray:
    """
...
def detect_runs(df: pd.DataFrame) -> List[Tuple[int, int]]:
    ...
    runs = [
        (run_boundaries[i], run_boundaries[i + 1])
        for i in range(len(run_boundaries) - 1)
    ]
    return runs


def pad_or_truncate_runs(
    run_grids: List[np.ndarray], target_steps: Optional[int] = None
) -> List[np.ndarray]:
    """
...
def multiple_run_grid(
    ...
    if pad_runs:
        ...
        if len(set(step_counts)) > 1:
            raise ValueError(
                "Runs have different step counts. Use pad_runs=True to standardize them."
            )
